...
public class DWGraph_DS implements directed_weighted_graph{
	...
	public void addNode(node_data n) {
		if(n == null) {throw new NullPointerException("can not add null to the graph");}	//make sure the node is not null
		
		if(!this.nodes.containsKey(n.getKey())) {			//check if have node with same key
			
		Node node = new Node(n.getKey() , n.getWeight() , n.getInfo() , n.getLocation());		//for to use at local function, need to convert the node to Node class 
	...
	/**
	 * add the edge src-dest with weight w, does noting if nodes does'nt exist in the graph of the edge already exist.
	 * if the edge exist, but with different weight, update the weight
	 * @param src
	 * @param dest
	...
	public void connect(int src, int dest, double w) {
		if(!this.nodes.containsKey(src) || !this.nodes.containsKey(dest) || w <= 0) {return;}			//check for valid input
		if(this.edges.containsKey(src+","+dest) && this.getEdge(src, dest).getWeight() == w) {return;}	//if the edge is in the graph, do nothing									//i	
	...
	public Collection<edge_data> getE(int node_id) {
		if(!this.nodes.containsKey(node_id)) {return null;}	//make sure the node is in the graph
	...
	public node_data removeNode(int key) {
		if(!this.nodes.containsKey(key)) {return null;}										//make sure the node is in the graph
	...
	public edge_data removeEdge(int src, int dest) {
		if(!this.edges.containsKey(src+","+dest)) {return null;}	//make sure the edge exist in the graph
