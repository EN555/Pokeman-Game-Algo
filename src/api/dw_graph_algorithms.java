...
/**
 * This interface represents a Directed (positive) Weighted Graph Theory
 * Algorithms including: 0. clone(); (copy) 1. init(graph); 2. isConnected(); //
 * strongly (all ordered pais connected) 3. double shortestPathDist(int src, int
 * dest); 4. List<node_data> shortestPath(int src, int dest); 5. Save(file); //
 * JSON file 6. Load(file); // JSON file
 *
...
public interface dw_graph_algorithms {
	 ...
	 * each other node. NOTE: assume directional graph (all n*(n-1) ordered pairs).
	 * 
	 * @return
	 */
