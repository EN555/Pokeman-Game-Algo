...
public class Ex2_Client {
	...
	public Ex2_Client(game_service game) {
		...
		this.arena = new Arena(this.game.getGraph(), this.game.getPokemons(), this.game.getAgents()); // update the
																										// arena field
	...
	public void moveAgents() {
		...
		for (CL_Agent agent : this.arena.getAgents()) {
			...
			if (agent.getDest() == -1) {
				...
				double min_dis = ga.shortestPathDist(agent.getSrc(), min_pok.getEdge().getSrc())
						+ min_pok.getEdge().getWeight();

				// check each pokemon
				for (CL_Pokemon pok : this.arena.getPokemons()) {
					if (!this.to_be_picked.contains(pok)) { // make sure no other agent is on it's way to pick this
															// pokemon
						double dis = ga.shortestPathDist(agent.getSrc(), pok.getEdge().getSrc())
								+ pok.getEdge().getWeight(); // calculate the distance to the pokemon
